...
void Help(void)
{
	...
	printf("\r\n  1            - Draw horizontal coloured lines ") ;
	// printf("\r\n  2            - Read voltages off of ADC0 ") ;
	// printf("\r\n  R            - Read bytes from Eeprom ") ;
	// printf("\r\n  W            - Write bytes to Eeprom ") ;
    printf(banner) ;
}
...
{
    char c,c1 ;
	unsigned int i2c_eeprom_start_address;
	unsigned int i2c_eeprom_size[6];
	unsigned int i2c_read_size;
	unsigned char i2c_write_data;
	
	unsigned short fxcount = 0;
	int wait_count = 0;
    ...
    while(1)    {
		...
		else if( c == (char)('1')) {             // draw horizontal lines to screen
			
			// for (fxcount = 2; fxcount < 480; fxcount++) {
				
			// WriteAPixel(fxcount,30, RED_X) ;					// write colour ‘2’ to [100,100]
			// printf("Read pixel value %d from %d\n",ReadAPixel(fxcount,30), fxcount);		// should read 2 back
			// }
			
			// WriteAPixel(fxcount,30, GREEN_X) ;					// write colour ‘2’ to [100,100]
			// printf("Read pixel value %d from %d\n",ReadAPixel(fxcount,30), fxcount);		// should read 2 back
			
			 DrawHLineCommand(100,100,200,100,RED_X) ;
			 DrawHLineCommand(300,250,50,250,RED_X) ;
			
			 DrawVLineCommand(100,300,100,400,RED_X) ;
			 DrawVLineCommand(200,470,200,250,RED_X) ;
			
			
			DrawAnyLineCommand(100,100,200,100,GREEN_X) ;
			DrawAnyLineCommand(300,150,50,150, GREEN_X) ;
			
			DrawAnyLineCommand(105,105,105,350,WHITE_X) ;
			DrawAnyLineCommand(305,325,305,237, WHITE_X) ;
			
			DrawAnyLineCommand(0,0,600,470, BLUE_X) ; 
			
			DrawAnyLineCommand(600, 400, 300, 470 CYAN_X) ;
			
			
		}
		
        // else if( c == (char)('1'))             // write voltages onto dac
            // Output_DAC_Voltage() ;
			
        // else if( c == (char)('2'))             // read voltages off adc0
            // Get_ADC0_Voltage() ;
			
        // else if( c == (char)('R'))  {          // read from i2c eeprom
			// printf( "\nEnter start address: " );
			// i2c_eeprom_start_address = Get8HexDigits(0) ;
			// printf("\nAddress %X selected", i2c_eeprom_start_address);	//apparently i need this here ...
			// printf( "\nEnter I2C Eeprom read size: " );
			// i2c_eeprom_size[5] = getchar(); 
			// i2c_eeprom_size[4] = getchar();
			// i2c_eeprom_size[3] = getchar();
			// i2c_eeprom_size[2] = getchar(); 
			// i2c_eeprom_size[1] = getchar();
			// i2c_eeprom_size[0] = getchar();
			// printf( "\n\n");
			// i2c_read_size =((i2c_eeprom_size[5] - 48)*100000) +((i2c_eeprom_size[4] - 48)*10000)+((i2c_eeprom_size[3] - 48)*1000)+ ((i2c_eeprom_size[2] - 48)*100) + ((i2c_eeprom_size[1] - 48)*10) + (i2c_eeprom_size[0]-48 );
			// RW_Eeprom_Bytes( i2c_eeprom_start_address, i2c_read_size, 0 , 1 );
        // }
		
        // else if( c == (char)('W'))  {          // read from i2c eeprom
			// printf( "\nEnter start address: " );
			// i2c_eeprom_start_address = Get8HexDigits(0) ;
			// printf("\nAddress %X selected", i2c_eeprom_start_address);	//apparently i need this here ...
			// printf( "\nEnter I2C Eeprom write size: " );
			// i2c_eeprom_size[5] = getchar(); 
			// i2c_eeprom_size[4] = getchar();
			// i2c_eeprom_size[3] = getchar();
			// i2c_eeprom_size[2] = getchar(); 
			...
			// i2c_eeprom_size[0] = getchar();
			// printf( "\nInput data to write: ");
			// i2c_write_data = Get2HexDigits(0) ;
			// printf("\nWrite data 0x%X\n", i2c_write_data);	//apparently i need this here ...
			
			// i2c_read_size =((i2c_eeprom_size[5] - 48)*100000) +((i2c_eeprom_size[4] - 48)*10000)+((i2c_eeprom_size[3] - 48)*1000)+ ((i2c_eeprom_size[2] - 48)*100) + ((i2c_eeprom_size[1] - 48)*10) + (i2c_eeprom_size[0]-48 );
			// RW_Eeprom_Bytes( i2c_eeprom_start_address, i2c_read_size, i2c_write_data , 0 );
        // }
...
void MemoryTest(void)
{
    ...
    // char c ;
    // int a;
    // //unsigned int check_val_1 = 0xFFFDFFFE;
    // //unsigned int check_val_2 = 0x00020001;
    // unsigned int check_val_1 = 0;
    // unsigned int check_val_2 = 0;

    // // counts how many reads and/or writes failed
    ...
    // // Make RamPtr point to the start address and iterate to the end address while adding 4 (respresenting address) each iteration
    // for ( RamPtr = Start; RamPtr < End && !failed_cnt ; RamPtr++ ) {
        // check_val_1 += 1;
        // check_val_2 = check_val_1 ^ 0xFFFFFFFF;
        // //printf( "\nPointer byte address is at 0x%x, ", RamPtr );

        // // Write the 1st check value, a 32-bit value to test the two word addresses,
        // // then read from the same address, then write the 2nd check value
        // // then read again.  Even one error will fail the test
        // *RamPtr = check_val_1;

        // if ( *RamPtr == check_val_1 ) {
                // //printf("Val1 write 0x%x: 0x%x", check_val_1 ,*RamPtr );
            // *RamPtr = check_val_2;

            // if ( *RamPtr == check_val_2 ) {
                // //printf("  Val2 write 0x%x: 0x%x", check_val_2 ,*RamPtr );
            // } else {
                // printf("\n\nFailed to write value of 0x%x to address 0x%x, read 0x%x instead\nStopping test . . .\n", check_val_2, RamPtr, *RamPtr);
                // failed_cnt++;
            // }

        // } else {
            // printf("\n\nFailed to write value of 0x%x to address 0x%x, read 0x%x instead\nStopping test . . .\n", check_val_1, RamPtr, *RamPtr);
            // failed_cnt++;
        // }

        // // count how many iterations there were
        // counter1++;

        // /*
        // // A hastily pieced together wait code for debugging my code since I can't use
        // // Sleep statements.
        // for ( a = 0; a < 5000000; a++ ) {
            // a = a + 1; // dummy statement
            // a = a - 1; // dummy statement
        // }
        // */
    ...
    // // Displayed how many write and/or reads failed vs total measurements.
    // printf( "\n\nTim's testing of the memory in the entered address range: 0x%x - 0x%x\n%d failed out of %d measurements.\n\n", Start, End, failed_cnt, counter1-1);
    // if ( !failed_cnt ) {
        // printf( "SUCCESS! Memory hardware works!\n\n" );
    // } else {
        // printf("FAILURE! Something is wrong with the hardware!!!\n\n" );
    // }
}
...
// clear memory in static ram at address 00010000 for 16k
    ...
    // // Initialize our I2C controller for Assignment 4
    // Init_I2C_Controller();
